...
def get_scalp_data(pair):
    try:
        ...
        h5  = yf.Ticker(symbol).history(period="1d",  interval="5m")
        h15 = yf.Ticker(symbol).history(period="5d",  interval="15m")
        if h5.empty or h15.empty:
            return None
        price = round(h5['Close'].iloc[-1], 5)
        ...
        rsi5   = round(100 - (100 / (1 + gain5 / loss5)), 1) if loss5 != 0 else 100.0

        ema9_5  = h5['Close'].ewm(span=9,  adjust=False).mean().iloc[-1]
        ema21_5 = h5['Close'].ewm(span=21, adjust=False).mean().iloc[-1]

        low14_5  = h5['Low'].rolling(14).min().iloc[-1]
        high14_5 = h5['High'].rolling(14).max().iloc[-1]
        ...
        macd5   = round((ema12_5 - ema26_5).iloc[-1], 5)
        signal5 = round((ema12_5 - ema26_5).ewm(span=9, adjust=False).mean().iloc[-1], 5)
        ...
        rsi15   = round(100 - (100 / (1 + gain15 / loss15)), 1) if loss15 != 0 else 100.0

        ema9_15  = h15['Close'].ewm(span=9,  adjust=False).mean().iloc[-1]
        ema21_15 = h15['Close'].ewm(span=21, adjust=False).mean().iloc[-1]

        low14_15  = h15['Low'].rolling(14).min().iloc[-1]
        high14_15 = h15['High'].rolling(14).max().iloc[-1]
        stoch15   = round(100 * (price - low14_15) / (high14_15 - low14_15), 1) if (high14_15 - low14_15) != 0 else 50.0

        recent = h15.tail(20)
        res = round(recent['High'].max(), 5)
        sup = round(recent['Low'].min(), 5)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not d:
        await q.edit_message_text("❌ خطأ، حاول مرة أخرى", reply_markup=back_keyboard())
        return
